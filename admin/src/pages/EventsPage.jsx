...
const EventsPage = () => {
    ...
    const fetchEvents = async () => {
        const res = await fetch('https://hotel-new-bp32.onrender.com/api/events');
    ...
    const handleAdd = async () => {
        ...
        try {
            await fetch('https://hotel-new-bp32.onrender.com/api/events', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ name, description, capacity, image })
            });
            setName("");
            setDescription("");
            setCapacity("");
            setImage("");
            fetchEvents();
            toast.success("Event venue added successfully");
        } catch(e) {
            toast.error("Failed to add event venue");
        }
    };
    ...
    return (
        <div className="min-h-screen bg-stone-950 p-6">
            ...
            <div className="bg-stone-900 p-6 rounded-lg mb-8 max-w-2xl border border-stone-800">
                <h2 className="text-xl text-gold-500 mb-4">Add Event Venue</h2>
                <div className="space-y-3">
                    ...
                    <input className="p-3 w-full bg-stone-800 text-white rounded border border-stone-700 outline-none focus:border-gold-500" placeholder="Capacity (e.g. 500 Guests)" value={capacity} onChange={e => setCapacity(e.target.value)} />
                    <div>
                    ...
                    <button onClick={handleAdd} className="mt-4 bg-gold-500 text-stone-900 font-semibold px-6 py-2 rounded hover:bg-gold-600">Add Event Venue</button>
                </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {events.map(e => (
                    <div key={e._id} className="bg-stone-900 rounded-lg overflow-hidden border border-stone-800">
                        {e.image ? (
                            <img src={e.image?.includes('http') || e.image?.includes('data:') ? e.image : `https://hotel-new-bp32.onrender.com/${e.image}`} alt={e.name} className="w-full h-48 object-cover" />
                        ) : (
                            <div className="w-full h-48 bg-stone-800 flex items-center justify-center text-stone-500">No Image</div>
                        )}
                        <div className="p-4 relative">
                            <h3 className="text-xl text-white font-serif">{e.name}</h3>
                            <p className="text-stone-400 text-sm mt-1">Capacity: {e.capacity}</p>
                            <button onClick={async () => {
                                if(window.confirm('Are you sure you want to delete this event venue?')) {
                                    try {
                                        await fetch(`https://hotel-new-bp32.onrender.com/api/events/${e._id}`, { method: 'DELETE' });
                                        fetchEvents();
                                        toast.success("Event venue deleted");
                                    } catch(e) {
